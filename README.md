...
VizGen 是一个类似 [Atoms](https://atoms.dev/) 能力的垂直化 Demo：用户上传 CSV 数据并用自然语言描述需求，Agent 流水线（Planner → Builder → Checker → Fixer → Render）自动完成图表规划、代码生成、配置校验与修复，最终产出一个**自包含的、可交互、可分享的数据看板网页应用**，并在工作台中实时预览。
...
**LLM 只输出结构化配置，看板 HTML 由确定性引擎渲染**——因此生成结果永远可解析、可校验、可兜底，Demo 不会因 LLM 波动而翻车。
