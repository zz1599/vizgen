...
7. 视口适配（重要）：应用会运行在高度约 600px 的预览窗口中。
   - 页面整体布局用 min-height:100vh 并允许滚动，禁止在 body 或最外层容器上使用 overflow:hidden 或固定 height 导致内容被裁剪。
   - 禁止用"固定高度容器 + flex 居中"承载可能超出一屏的内容（内容比容器高时顶部会被裁掉且无法滚动到）；安全做法是给内容元素设 margin:auto 或使用 place-items:safe center。
   - 游戏棋盘/主区域尺寸要自适应视口（优先使用 min(视口宽高相关单位) 缩放），标题、按钮、说明等所有 UI 在小窗口下都完整可见；确实放不下时必须保证整页可滚动查看。`;
